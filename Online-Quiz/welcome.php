...
<!-- Timer Display -->
<div id="timerBox" style="display:none; position: fixed; top: 5px; right: 10px; background: #ffc; padding: 10px; border-radius: 6px; border: 2px solid #333; font-weight: bold; z-index: 999;">
    ⏳ Time Left: <span id="timer">00:20</span>
</div>

<!-- Timer Script -->
<script>
let totalSeconds = localStorage.getItem("quizTimer") || 20;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (url.includes("q=quiz") && url.includes("step=2")) {
        document.getElementById("timerBox").style.display = "block";

        // Reset timer at first question
        if (url.includes("&n=1")) {
            totalSeconds = 20;
        ...
        let countdown = setInterval(function () {
            ...
            if (totalSeconds <= 0) {
                ...
                localStorage.setItem("quizTimer", 0);

                alert("⏰ Time is over! Submitting your quiz...");
                ...
                if (form) {
                    // ✅ Create a hidden input to trigger result on server
                    let hiddenInput = document.createElement("input");
                    hiddenInput.type = "hidden";
                    hiddenInput.name = "autotimeup";
                    hiddenInput.value = "1";
                    form.appendChild(hiddenInput);

                    form.submit(); // ✅ submit the current question

                    // ✅ Wait and redirect to result page after submission
                    setTimeout(() => {
                        window.location.href = `welcome.php?q=result&eid=${eid}`;
                    }, 2000);
                } else {
                    // fallback in case form not found
                    window.location.href = `welcome.php?q=result&eid=${eid}`;
                }
            }
        }, 1000);
